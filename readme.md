...
* Написание функции ExecutePipeline, которая обеспечивает нам конвейерную обработку функций-воркеров. Эти функции что-то делают.
...
* Если делать в лоб, линейно - для 7 элементов это займёт почти 57 секунд, следовательно, надо это как-то распараллеливать
...
### Подсказки:
* Задание построено так, чтобы хорошо разобраться со всем материалом лекции, т.е. вдумчиво посмотреть примеры и применить их на практике. Искать по гуглу или stackoverflow ничего не надо
...
* Ответ на вопрос "мне нужны результаты предыдущих вычислений?" помогают распараллеливать SingleHash и MultiHash
* Хорошо помогает нарисовать схему расчетов. И перечитать текст задания после того как вы начали немного разбираться с тестами
...
Тему с асинхроном спрашивают на всех собесах, так что несмотря на то, что домашка сложная, крайне рекомендуется ее все же сделать.